...
void worker_static(Image& image, int thread_id, int num_threads, int& pixels_inside) {
    ...
    for (int row = start_row; row < end_row; row++) {
        for (int col = 0; col < image.width; ++col) {
            ...
            pixels_inside_mutex.lock();
            if (mandelbrot_kernel(c, pixel, color)) {
                pixels_inside++;
            }
            pixels_inside_mutex.unlock();
...
int main(int argc, char **argv)
{
    ...
    if (num_threads <= 0 || num_threads > 32) {
        std::cerr << "Invalid number of threads. Please use a value between 1 and 32." << std::endl;
        return 1;
    }
